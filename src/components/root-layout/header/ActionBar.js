...
const ActionBar = () => {
  ...
  return (
    <div className={`hidden lg:block bg-[#010101] rounded-3xl ${isHomePage ? 'bg-opacity-0' : 'bg-opacity-0'}`}>
      <div className="container mx-auto flex justify-between items-center pt-6 text-white px-5 ">
        {/* Sol Taraf - Sosyal Medya İkonları */}
        <div className="flex gap-4 items-center ">
          {socialLinks.map(({ href, icon: Icon }) => (
            <a href={href} target="_blank" rel="noopener noreferrer" key={href} className="rounded-full ">
        ...
        <div className="flex gap-8 items-center font-medium">
          <div className="relative group">
            ...
            <span
              className="block h-0.5 bg-white absolute  -bottom-1 transform origin-center scale-x-0 group-hover:scale-x-100 transition-transform duration-300 "
              style={{ width: "5.3rem", transformOrigin: "center" }}
            ></span>
          </div>
          ...
          <div className="relative group">
            ...
            <span
              className="block h-0.5 rounded-full bg-white absolute -bottom-1 transform origin-center scale-x-0 group-hover:scale-x-100 transition-transform duration-300 "
              style={{ width: "7.65rem", transformOrigin: "center" }}
            ></span>
          </div>
